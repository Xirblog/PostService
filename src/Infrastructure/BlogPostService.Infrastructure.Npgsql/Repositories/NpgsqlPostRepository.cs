...
using BlogPostService.Infrastructure.Npgsql.Models;
using Dapper;
using Npgsql;
...
namespace BlogPostService.Infrastructure.Npgsql.Repositories;
...
public class NpgsqlPostRepository : IPostRepository
{
    ...
    public async Task<Post> AddAsync(Post post, CancellationToken cancellationToken)
    {
        ...
        PostModel model = await connection.QuerySingleAsync<PostModel>(
            sql,
            new
            {
                post.Name,
